...
<div class="container mx-auto max-w-7xl py-8">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-xl font-bold">Список услуг</h1>
        <a href="{{ route('services.create') }}" class="btn btn-sm btn-primary button-style" style="font-weight: 400;">+ Создать услугу</a>
    </div>
    ...
    <div class="bg-white border rounded shadow-sm overflow-hidden" style="font-family: 'Rubik'; font-weight: 300;">
        <table class="w-full text-sm text-left">
            ...
            <tbody>
                @forelse($services as $service)
                    <tr class="border-b hover:bg-gray-50">
                        ...
                        <td class="p-3 text-right flex justify-end gap-2">
                            <a href="{{ route('services.edit', $service) }}" title="Редактировать" style="align-items: center; display: flex;">
                            ...
                            <form action="{{ route('services.destroy', $service) }}" method="POST" onsubmit="return confirm('Удалить услугу?')" style="align-items: center; display: flex;">
                                @csrf
                                @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="6" class="p-6 text-center text-gray-500">Нет услуг.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    @if ($services->hasPages())
        <div class="flex items-center justify-between mt-6 px-4 py-3 bg-white text-sm text-gray-700">
            <div>
                Показано с {{ $services->firstItem() }} по {{ $services->lastItem() }} из {{ $services->total() }}
            </div>
