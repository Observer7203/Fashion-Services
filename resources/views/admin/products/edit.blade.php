...
<div class="container py-6">
    ...
    <form method="POST" action="{{ route('products.update', $product) }}">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="category" class="w-full border p-2" value="{{ old('category', $product->category) }}" required>
        </div>
        ...
        <div class="mb-4">
            ...
            <input type="number" step="0.01" name="price" class="w-full border p-2" value="{{ old('price', $product->price) }}" required>
        </div>

        <div class="mb-4">
            <label class="block font-semibold">Медиа (ссылки через запятую)</label>
            <input type="text" name="media[]" class="w-full border p-2"
                   value="{{ is_array($product->media) ? implode(',', $product->media) : '' }}">
        </div>

        <div class="mb-4">
            <label class="block font-semibold">Характеристики (JSON)</label>
            <textarea name="attributes" class="w-full border p-2" rows="2">{{ old('attributes', json_encode($product->attributes)) }}</textarea>
        </div>
