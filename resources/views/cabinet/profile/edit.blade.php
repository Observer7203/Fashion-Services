...
<div class="bg-gray-100 py-8 mb-8 border-b" style=" background: url({{ asset('images/bg-breadcrumbs2.JPG') }}); background-size: cover; height: 500px;">
    <div class="container mx-auto max-w-7xl py-8 flex gap-8" style="height: inherit; flex-direction: column; justify-content: center; margin-left: 220px;">
        <h1 class="text-3xl font-bold mb-2" style="font-size: 49px; color: white; font-weight: 800;">PROFILE</h1>
        <nav class="text-sm text-gray-600" style="font-size: 20px; color: white;">
            <a href="{{ route('home') }}" class="hover:underline">Home</a> > Profile
        </nav>
    </div>
</div>
...
<script>
 ...
 <aside class="" style="margin-bottom: 25px;">
        <div class="w-64 bg-white border rounded shadow-sm p-4 space-y-4" style="margin-bottom: 25px; margin-top: 25px; margin-left: 25px;">
        <div class="text-left">
...
<div class="flex-1 grid grid-cols-2 gap-8" style="margin-top: 25px; margin-left: 25px; margin-right: 25px;">
    <h1 class="text-2xl font-semibold mb-6" style="font-size: 20px;">Редактирование Профиля</h1>
    ...
    <form method="POST" action="{{ route('cabinet.profile.update') }}" class="space-y-6">
        ...
        <div>
            ...
            <input id="name" name="name" type="text" value="{{ old('name', auth()->user()->name) }}" required
                class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm-sm p-2">
        </div>
        ...
        <div class="pt-4">
            <button type="submit"
                class="button bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">Сохранить изменения</button>
        </div>
    </form>
</div>
</div>
